...
def build_report(baseline_usd: float, optimized_usd: float, levers: dict,
    ...
    """Return an exhaustive markdown cost-optimization report adhering to Rubric C standards."""
    savings = baseline_usd - optimized_usd
    pct = (savings / baseline_usd * 100.0) if baseline_usd > 0 else 0.0
    total_levers = sum(levers.values()) if sum(levers.values()) > 0 else 1.0
    ...
    lines = [
        ...
        "|---|:---:|:---:|:---:|:---:|",
        f"| **Monthly Total Spend** | **${baseline_usd:,.0f}** | **${optimized_usd:,.0f}** | **${savings:,.0f}** | **{pct:.1f}%** |",
        ...
        "",
        "The total projected savings of **$" + f"{savings:,.0f}/month" + f" ({pct:.0f}%)** is broken down across four distinct, non-overlapping architectural levers:",
        "",
    ...
    for name, amount in levers.items():
        share = (amount / total_levers) * 100.0
        src = "M3" if "Purchasing" in name else ("M2" if "Inference" in name else "M1")
        cx = "Low" if "idle" in name.lower() or "right" in name.lower() else ("Medium" if "infer" in name.lower() else "Low-Med")
        payback = "Immediate (<1 mo)" if "Purchasing" in name or "idle" in name.lower() else "< 2 weeks"
        lines.append(f"| **{name}** | {src} | **${amount:,.0f}** | **{share:.1f}%** | {cx} | {payback} |")
    ...
    if sustainability:
        lines += [
            f"- **Energy per query (800 tokens median):** {sustainability.get('wh_per_query', 0):.2f} Wh",
            f"- **Carbon per query (us-east-1 grid):** {sustainability.get('carbon_g', 0):.3f} gCO2e",
            f"- **Optimal deployment region:** {sustainability.get('best_region', 'europe-north1')} (Lowest carbon intensity)",
        ]
    else:
        lines += [
